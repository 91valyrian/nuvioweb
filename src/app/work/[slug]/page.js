...
export default function WorkDetail({ params }) {
  ...
  return (
    <article className="mx-auto max-w-[1200px] py-12 px-5">
      {/* 헤더 */}
      <header className="mb-10">
        <h1 className="text-3xl md:text-5xl font-bold tracking-tight">{work.title}</h1>
        <p className="mt-2 text-neutral-500">{work.client} · {work.year}</p>
      </header>
      ...
      {/* 요약 */}
      {work.summary && (
        <p className="mb-12 text-lg leading-relaxed text-neutral-700">{work.summary}</p>
      )}
